...
{ 
	...
	while((func = (RooAbsArg*)coefIter->Next())) {
		if (!dynamic_cast<RooAbsReal*>(func)) {
			coutE(InputArguments) << "ERROR: :Width_conv(" << GetName() << ") funcficient " << func->GetName() << " is not of type RooAbsReal" << std::endl;
			assert(0);
		}
		_coefList.add(*func) ;
	}
...
//	std::cout << xgen_max <<"\t"<<xgen_min<<std::endl;
	...
	double recow = (xreco_max-xreco_min)/350.;
	eff_bkg = effbkg; 
	eff_sig = effsig; 


	for(int j=0;j<500;j++){
		double xgenval = xgen_min+  j*(xgen_max-xgen_min)/500.;
		...
		for(int i=0;i<351;i++){
			xrecocopy->setVal(xreco_min+  i*(xreco_max-xreco_min)/350.);
...
{ 
	...
	double genwid = (xgen_max-xgen_min)/500.;
	...
	int  re = (xreco-xreco_min)/((xreco_max-xreco_min)/350.); 

	{
		...
		for(int k=0;k<100;k++){
			double cvalue = mean-10*width + k*width*0.2;
		...
		case 4: 
			{
...

				{
					...
					for(int k=0;k<100;k++){
						...
						integral+= gen_s*0.2*width*integral_store[j];
					}
